...
public sealed class SessionSummary : IDisposable
{
    ...
    private string BuildPrompt(string transcript, string template) =>
        ...
        - Preserve the template's markdown structure exactly: heading levels, table layout, list ordering, table column count, and any fenced code blocks. Fill every cell — never output an empty "|" column.
        - The "Highlight Reel Video Prompt (MiniMax H3)" section must contain a filled fenced code block with exactly three non-empty lines, in this order: `integrated_multimodal_description:`, `overall_soundscape:`, `non_diegetic_music:`. The first line of `integrated_multimodal_description:` must start with the exact token `[Shot 1]` followed by a single space — no preamble, no quotes, no lead-in sentence. Each later shot begins with `[Shot N] At MM:SS.sss,` (two-digit minute, three-digit second millis, strictly increasing timestamps inside the 15-second window). The full prompt must stay under 7,000 Unicode code points; drop shots from the end rather than abbreviating. Do not mention MiniMax, H3, "this video", the duration, or the aspect ratio inside the code block.
